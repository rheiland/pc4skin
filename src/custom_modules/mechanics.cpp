...
void parietal_epithelial_mechanics( Cell* pCell, Phenotype& phenotype, double dt )
{
    ...
    double dist_to_membrane = circle_dist(xpos,ypos);
    ...
    if (pCell->custom_data[idx_attached] == 0.0)  // not attached (custom_data are double (or std::string))
    {
        ...
        if (dist_to_membrane >  R_a)  // beyond membrane, but within attachment radius
        // if (circle_dist(xpos,ypos) <  0.01)  // within attachment radius
        {
